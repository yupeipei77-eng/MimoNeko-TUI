...
- Provide a stable string for `reasonforge version`.

## Boundaries

- This package has no dependency on configuration, runtime state, or Git metadata.
- Release builds may override `Version` with `go build -ldflags "-X github.com/reasonforge/reasonforge/internal/version.Version=vX.Y.Z"`.
