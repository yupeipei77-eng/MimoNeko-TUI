...
Phase 6 implements the Patch Review and Validation Pipeline, enabling ReasonForge to automatically perform structured review before applying a patch:

```
PatchPreview → RuleBasedReview → RiskScoring → TestValidation → Optional ModelReview → PatchReviewReport → Recommendation
```
...
3. Run `RuleBasedReviewer.Review` → findings
4. Run `RiskScorer.Score` → risk score
5. Optionally run `ValidationRunner.Validate` → validation result
6. Optionally run `ModelReviewer.Review` → model review result
...
| Binary files | critical/warning | Not allowed → critical; allowed → warning |
| Sensitive paths | critical | `.env`, `*.pem`, `*.key`, `.git`, `.reasonforge` |
...
**Safety boundaries:**
- `UseModelReview=false` → never calls `ModelRouter`
- If `PatchPreview.Violations` non-empty → raw diff NOT sent to model
- If diff is redacted marker → original diff NOT requested
...
### `reasonforge patch validate <worktree_id>`

Execute rule review + test validation (no model review):

```sh
reasonforge patch validate wt_xxx --test-command go-test
reasonforge patch validate wt_xxx --max-output-bytes 32768 --timeout-seconds 60
```
...
### `reasonforge patch review <worktree_id>`

Execute full review pipeline with optional model review and test validation:

```sh
reasonforge patch review wt_xxx --model-review
reasonforge patch review wt_xxx --test-command go-test --model-review
reasonforge patch review wt_xxx --no-tests
```
...
Both files are optional. Missing values use safe defaults. `reasonforge init` generates them. `reasonforge doctor` checks them.
...
2. **Multi-Agent Workflow**: Planner → Coder → Reviewer loop, where Reviewer uses `PatchReviewReport` to decide whether changes need revision
...
The `PatchReviewManager` interface is designed to be composable — Phase 7 agents can use it directly or wrap it with additional logic.
