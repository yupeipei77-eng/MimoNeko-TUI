...
The Tool Runtime is ReasonForge's secure local tool execution layer. It provides a unified, safety-guarded pipeline for all tool operations:
...
- Rejects .git, .reasonforge, sensitive paths
...
**Default path:** `.reasonforge/logs/tools.jsonl`
...
- Audit finish failure → tool result returned with error
...
### reasonforge tool-run

Executes a tool with arguments. Subject to all safety policies.

```sh
reasonforge tool-run file_read --path README.md
reasonforge tool-run git_diff
reasonforge tool-run test_run --command_name go-test
reasonforge tool-run file_write --path output.txt --content "hello" --dry-run
```
