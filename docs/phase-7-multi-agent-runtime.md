...
Phase 7 implements ReasonForge's minimal multi-agent collaboration runtime. It orchestrates three specialized agents in an iteration loop:
...
Path: `.reasonforge/checkpoints/multi_agent_runs.jsonl`
...
```sh
reasonforge multi-run "fix typo in README" [flags]
```
...
- Does NOT auto-push
- If approved, suggests: `reasonforge patch apply <worktree_id>`
