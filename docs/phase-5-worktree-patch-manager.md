...
Key behaviors:
- Worktrees are created under `.reasonforge/worktrees/<task_id>/<worktree_id>`
- Branch naming: `reasonforge/<sanitized_task_id>/<short_id>`
- Uses `git worktree add -b` to create isolated directories
- Registry stored as append-only JSONL at `.reasonforge/worktrees/registry.jsonl`
...
- Preview checks hard-coded deny list (.git, .reasonforge, .env, *.pem, *.key)
...
2. Worktree path must be under `.reasonforge/worktrees`
...
8. Patch apply refuses to modify .git, .reasonforge, .env, *.pem, *.key
...
Both files are optional. Missing files use safe defaults. `reasonforge init` creates them with default values.
...
```sh
reasonforge run --goal "fix typo in README" --worktree
```
...
- `review with: reasonforge patch preview wt_xxxx`
...
```sh
reasonforge patch preview wt_xxxx
```
...
```sh
reasonforge patch apply wt_xxxx
```

Behavior: checks violations, verifies clean main, applies diff (no commit)

```sh
reasonforge patch apply wt_xxxx --dry-run
```
...
```sh
reasonforge patch discard wt_xxxx
```
