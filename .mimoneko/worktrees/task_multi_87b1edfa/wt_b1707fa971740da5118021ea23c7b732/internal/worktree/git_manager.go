...
type GitWorktreeManager struct {
	registry      *Registry
	branchPrefix  string
	worktreeRoot  string // relative path under repo, e.g. ".nekonomimo/worktrees"
	maxActive     int
	keepFailed    bool
	keepCancelled bool
}
...
type GitWorktreeManagerConfig struct {
	// BranchPrefix is the prefix for worktree branch names (default: "NekoMIMO").
	BranchPrefix string
...
func NewGitWorktreeManager(registry *Registry, cfg GitWorktreeManagerConfig) *GitWorktreeManager {
	...
	return &GitWorktreeManager{
		...
		worktreeRoot:  ".nekonomimo/worktrees",
...
func (m *GitWorktreeManager) Create(ctx context.Context, req CreateWorktreeRequest) (WorktreeInfo, error) {
	...
	if !strings.HasPrefix(absWtPath, absExpectedRoot+string(os.PathSeparator)) {
		return WorktreeInfo{}, fmt.Errorf("worktree: path escapes .nekonomimo/worktrees")
	}
...
// .nekonomimo/worktrees directory.
func IsWorktreePathSafe(repoRoot, wtPath string) bool {
	absWtPath, err := filepath.Abs(wtPath)
	if err != nil {
		return false
	}
	absExpectedRoot, err := filepath.Abs(filepath.Join(repoRoot, ".nekonomimo", "worktrees"))
	if err != nil {
		return false
	}
