...
import (
	...
	"github.com/nekonomimo/nekonomimo/internal/neko/animation"
	"github.com/nekonomimo/nekonomimo/internal/neko/branding"
	"github.com/nekonomimo/nekonomimo/internal/neko/layout"
)
...
func (c *Console) runGoal(ctx context.Context, goal string) {
	...
	if result.WorktreeID != "" {
		...
		fmt.Fprintf(&msg, "NekoMIMO patch apply %s\n", result.WorktreeID)
	}
...
func defaultLocalChatReply(message string) string {
	...
	if lower == "hi" || lower == "hello" || lower == "你好" || lower == "您好" {
		return "你好，我�?NekoForge。可以直接和我聊天；需要执行代码任务时，用 /run <goal>�?
	}
	return "我在。直接输入是聊天；执行代码任务请�?/run <goal>�?
}
