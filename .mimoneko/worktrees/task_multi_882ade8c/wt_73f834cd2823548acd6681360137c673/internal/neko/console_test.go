...
func TestNekoRendersBranding(t *testing.T) {
	...
	var out bytes.Buffer
	RenderHeader(&out, session)
	if !strings.Contains(out.String(), "NekoForge") || !strings.Contains(out.String(), "powered by NekoMIMO") {
...
func TestNekoShowsPatchNextSteps(t *testing.T) {
	...
	for _, want := range []string{"/preview wt_next", "/review wt_next", "/discard wt_next", "NekoMIMO patch apply wt_next"} {
