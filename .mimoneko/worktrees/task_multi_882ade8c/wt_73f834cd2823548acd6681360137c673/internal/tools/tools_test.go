...
func TestFileReadRejectsNekoMIMODir(t *testing.T) {
	root := t.TempDir()
	tool := &FileReadTool{}

	// .nekonomimo/logs/tools.jsonl
	resp, _ := tool.Run(context.Background(), ToolRequest{
		RepoRoot: root,
		Args:     map[string]string{"path": ".nekonomimo/logs/tools.jsonl"},
	})
	if resp.Success {
		t.Fatal("file_read should reject .nekonomimo/logs/tools.jsonl")
	}

	// .nekonomimo itself
	resp, _ = tool.Run(context.Background(), ToolRequest{
		RepoRoot: root,
		Args:     map[string]string{"path": ".nekonomimo"},
	})
	if resp.Success {
		t.Fatal("file_read should reject .nekonomimo")
	}
}
