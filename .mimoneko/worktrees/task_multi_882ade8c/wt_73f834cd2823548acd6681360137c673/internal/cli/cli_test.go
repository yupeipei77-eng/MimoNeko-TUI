...
func TestVersion(t *testing.T) {
	...
	if got := strings.TrimSpace(stdout.String()); got != "NekoMIMO 0.1.0-dev" {
...
func TestInitThenDoctor(t *testing.T) {
	...
	if !strings.Contains(initOut.String(), "Initialized NekoMIMO") {
	...
	if !strings.Contains(doctorOut.String(), "NekoMIMO doctor OK") {
...
func TestInitRepairDoesNotOverwriteModelsConfig(t *testing.T) {
	...
	modelsPath := filepath.Join(root, ".nekonomimo", "models.yaml")
...
func TestDoctorSuggestsInitRepair(t *testing.T) {
	...
	if !strings.Contains(stderr.String(), "NekoMIMO init --repair") {
...
func TestNoArgsReturnsUsageError(t *testing.T) {
	...
	if !strings.Contains(stderr.String(), "Usage: NekoMIMO <command>") {
...
func TestModelsCommand(t *testing.T) {
	...
	output := stdout.String()
	if !strings.Contains(output, "NekoMIMO Models") {
		t.Fatalf("models output = %q, want NekoMIMO Models header", output)
	}
	...
	if !strings.Contains(output, "api_key_env=NekoMIMO_API_KEY") {
...
func TestNekoMIMONekoAlias(t *testing.T) {
	root := setupModelCommandRoot(t)
	var stdout bytes.Buffer
	code := Run([]string{"neko", "--dir", root, "--no-color"}, Env{Stdout: &stdout, Stdin: strings.NewReader("/exit\n")})
	if code != 0 {
		t.Fatalf("NekoMIMO neko code = %d, output = %q", code, stdout.String())
	}
...
func TestModelTestAcceptsPrompt(t *testing.T) {
	...
	var stdout bytes.Buffer
	code := Run([]string{"model", "test", "--dir", root, "--provider", "test", "--model", "test-model", "--prompt", "只回�?OK"}, Env{Stdout: &stdout})
	if code != 0 {
		t.Fatalf("model test code = %d", code)
	}
	if !strings.Contains(gotBody, "只回�?OK") {
...
func readModelsYAMLForTest(t *testing.T, root string) string {
	t.Helper()
	content, err := os.ReadFile(filepath.Join(root, ".nekonomimo", "models.yaml"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestToolsCommand(t *testing.T) {
	...
	output := stdout.String()
	if !strings.Contains(output, "NekoMIMO Tools") {
		t.Fatalf("tools output = %q, want NekoMIMO Tools header", output)
	}
...
func TestToolRunFileWriteDryRun(t *testing.T) {
	...
	auditPath := filepath.Join(root, ".nekonomimo", "logs", "tools.jsonl")
...
func TestBuildAgentDependenciesNoAPIKeyLeak(t *testing.T) {
	...
	t.Setenv("NekoMIMO_API_KEY", "sk-test-secret-key-do-not-leak")
...
func TestInitCreatesWorktreeAndPatchConfig(t *testing.T) {
	...
	worktreeYAML := filepath.Join(root, ".nekonomimo", "worktree.yaml")
	...
	patchYAML := filepath.Join(root, ".nekonomimo", "patch.yaml")
...
func TestPatchApplyDirtyMainRejects(t *testing.T) {
	...
	runGitCmd(t, root, "config", "user.email", "test@NekoMIMO.dev")
...
func TestCLIDoesNotLeakAPIKey(t *testing.T) {
	t.Setenv("NekoMIMO_API_KEY", "sk-super-secret-key-12345")
...
func loadPatchCLIEvents(t *testing.T, root string) []events.RunEvent {
	t.Helper()
	path := filepath.Join(root, ".nekonomimo", "events", "run_events.jsonl")
...
func TestPatchReviewDoesNotLeakSensitiveDiffInEvents(t *testing.T) {
	...
	eventStorePath := filepath.Join(root, ".nekonomimo", "events", "run_events.jsonl")
...
func TestPatchCLIEventsDisabledUsesNoop(t *testing.T) {
	...
	eventsYAML := filepath.Join(root, ".nekonomimo", "events.yaml")
...
func TestDashboardRequiresEventsEnabled(t *testing.T) {
	...
	eventsPath := filepath.Join(root, ".nekonomimo", "events.yaml")
	os.WriteFile(eventsPath, []byte("enabled: false\nstore_path: .nekonomimo/events/run_events.jsonl\n"), 0o600)
...
func TestDashboardHandlesEmptyEventStore(t *testing.T) {
	...
	eventsDir := filepath.Join(root, ".nekonomimo", "events")
