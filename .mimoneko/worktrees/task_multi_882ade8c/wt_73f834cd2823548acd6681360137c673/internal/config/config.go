...
func (cfg *Root) applyWorktreeDefaults() {
	...
	if cfg.Worktree.Root == "" {
		cfg.Worktree.Root = ".nekonomimo/worktrees"
	}
	if cfg.Worktree.BranchPrefix == "" {
		cfg.Worktree.BranchPrefix = "NekoMIMO"
	}
...
func (cfg *Root) applyEventsDefaults() {
	if cfg.Events.StorePath == "" {
		cfg.Events.StorePath = ".nekonomimo/events/run_events.jsonl"
	}
...
var defaultScaffoldFiles = []scaffoldFile{
	{
		...
		Body: `You are NekoMIMO, a safe local coding agent.
...
cache:
  registry_path: .nekonomimo/cache/prefixes.jsonl
  estimated_ttl: 1h
budget:
  warn_ratio: 0.8
  block_ratio: 1.0
