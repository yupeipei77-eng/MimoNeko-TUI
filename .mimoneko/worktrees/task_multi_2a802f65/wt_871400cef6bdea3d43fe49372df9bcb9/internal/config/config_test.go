...
func TestInitCreatesSystemPrompt(t *testing.T) {
	...
	if !strings.Contains(string(content), "You are NekoMIMO") {
		t.Fatalf("system prompt = %q, want NekoMIMO default", string(content))
	}
}
	...
	return &Root{
		...
		Prefix: PrefixConfig{
			...
			Cache: PrefixCacheConfig{
				RegistryPath: ".nekonomimo/cache/prefixes.jsonl",
			},
			Budget: BudgetConfig{
				WarnRatio:  0.8,
				BlockRatio: 1.0,
