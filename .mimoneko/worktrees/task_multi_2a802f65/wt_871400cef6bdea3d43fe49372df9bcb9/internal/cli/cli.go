...
import (
	...
	"github.com/nekonomimo/nekonomimo/internal/dashboard"
	"github.com/nekonomimo/nekonomimo/internal/events"
	"github.com/nekonomimo/nekonomimo/internal/modelprofile"
	"github.com/nekonomimo/nekonomimo/internal/modelrouter"
	"github.com/nekonomimo/nekonomimo/internal/multiagent"
	"github.com/nekonomimo/nekonomimo/internal/neko"
	...
	webserver "github.com/nekonomimo/nekonomimo/internal/server"
	"github.com/nekonomimo/nekonomimo/internal/task"
	"github.com/nekonomimo/nekonomimo/internal/tools"
	"github.com/nekonomimo/nekonomimo/internal/validation"
	"github.com/nekonomimo/nekonomimo/internal/version"
...
func runInit(args []string, env Env) int {
	...
	repair := fs.Bool("repair", false, "repair missing NekoMIMO scaffolding without overwriting existing files")
	if err := fs.Parse(args); err != nil {
		return 2
	}
	if rejectExtraArgs(fs, env) {
	...
	if *repair {
		fmt.Fprintf(env.Stdout, "Repaired NekoMIMO scaffolding at %s\n", config.ConfigDir(root))
	} else if len(result.Created) == 0 {
		fmt.Fprintf(env.Stdout, "NekoMIMO already initialized at %s\n", config.ConfigDir(root))
	} else {
		fmt.Fprintf(env.Stdout, "Initialized NekoMIMO at %s\n", config.ConfigDir(root))
	}
...
func printInitNextSteps(w io.Writer) {
	...
	fmt.Fprintln(w, "   NekoMIMO model setup")
	fmt.Fprintln(w, "2. Test the model:")
	fmt.Fprintln(w, "   NekoMIMO model test")
	fmt.Fprintln(w, "3. Run a safe task:")
	fmt.Fprintln(w, "   NekoMIMO run --goal \"Reply OK\" --dry-run")
	fmt.Fprintln(w, "4. Start dashboard:")
	fmt.Fprintln(w, "   NekoMIMO serve")
...
func runDoctor(args []string, env Env) int {
	...
	if len(missingSources) > 0 {
		...
		fmt.Fprintln(env.Stderr, "NekoMIMO init --repair")
		return 1
	}

	fmt.Fprintln(env.Stdout, "NekoMIMO doctor OK")
...
func printUsage(w io.Writer) {
	fmt.Fprintln(w, "Usage: NekoMIMO <command>")
	fmt.Fprintln(w)
	fmt.Fprintln(w, "Commands:")
	fmt.Fprintln(w, "  version      Print version information")
	fmt.Fprintln(w, "  init         Create local .nekonomimo config files")
	fmt.Fprintln(w, "  doctor       Validate local NekoMIMO configuration")
...
func printNekoUsage(w io.Writer) {
	...
	fmt.Fprintln(w, "       NekoMIMO neko [same flags]")
	fmt.Fprintln(w)
	fmt.Fprintln(w, "NekoForge is a local terminal console powered by NekoMIMO.")
...
func runModels(args []string, env Env) int {
	...
	fmt.Fprintln(env.Stdout, "NekoMIMO Models")
	fmt.Fprintf(env.Stdout, "default_model=%s\n", cfg.Models.Routing.DefaultModel)
...
func runModel(args []string, env Env) int {
	if len(args) == 0 {
		...
		fmt.Fprintln(env.Stderr, "Usage: NekoMIMO model <setup|list|discover|enrich|test|use|remove>")
		return 2
	}
...
func runModelList(args []string, env Env) int {
	...
	fmt.Fprintln(env.Stdout, "NekoMIMO Model Profiles")
...
func runTools(args []string, env Env) int {
	...
	fmt.Fprintln(env.Stdout, "NekoMIMO Tools")
...
func runAgent(args []string, env Env) int {
	...
	fmt.Fprintf(env.Stdout, "NekoMIMO Agent\n")
	...
	if result.PatchPreview != nil {
		...
		fmt.Fprintf(env.Stdout, "  review with: NekoMIMO patch preview %s\n", result.WorktreeID)
	}
...
func runMultiAgent(args []string, env Env) int {
	...
	if !hasGoalFlag && !hasPositionalGoal {
		...
		fmt.Fprintln(env.Stderr, "Usage: NekoMIMO multi-run --goal \"fix typo in README\"")
		fmt.Fprintln(env.Stderr, "   or: NekoMIMO multi-run \"fix typo in README\"")
		return 2
	}
	...
	fmt.Fprintf(env.Stdout, "NekoMIMO Multi-Agent\n")
	...
	if result.State == multiagent.MultiAgentStateSucceeded && result.WorktreeID != "" {
		...
		fmt.Fprintf(env.Stdout, "  NekoMIMO patch apply %s\n", result.WorktreeID)
	}
...
// runPatchList lists all worktrees managed by NekoMIMO.
func runPatchList(args []string, env Env) int {
	...
	fmt.Fprintln(env.Stdout, "NekoMIMO Worktrees")
...
func runRuns(args []string, env Env) int {
	...
	if !cfg.Events.Enabled {
		fmt.Fprintln(env.Stderr, "events system is disabled; enable in .nekonomimo/events.yaml")
		return 1
	}
	...
	fmt.Fprintln(env.Stdout, "NekoMIMO Runs")
...
func runRunEvents(args []string, env Env) int {
	...
	fmt.Fprintln(env.Stdout, "NekoMIMO Run Events")
...
// Usage:
//
//	NekoMIMO dashboard [--limit N] [--run <run_id>] [--watch]
//
// Security:
...
func runDashboard(args []string, env Env) int {
	...
	if !cfg.Events.Enabled {
		fmt.Fprintln(env.Stderr, "Events system is disabled. Enable it in .nekonomimo/events.yaml to use the dashboard.")
		return 1
	}
	...
	if err != nil {
		...
		fmt.Fprintln(env.Stderr, "Hint: run a command (e.g. 'NekoMIMO run --goal ...') to create events first.")
		return 1
	}
	defer cleanup()
...
func runDashboardList(env Env, store *events.JSONLRunEventStore, limit int) int {
	...
	if len(runs) == 0 {
		fmt.Fprintln(env.Stdout, "NekoMIMO Dashboard")
		fmt.Fprintln(env.Stdout)
		fmt.Fprintln(env.Stdout, "No runs found. Run a command to create events:")
		fmt.Fprintln(env.Stdout, "  NekoMIMO run --goal \"your task\"")
		return 0
	}
...
func runServe(args []string, env Env) int {
	...
	fmt.Fprintf(env.Stdout, "NekoMIMO Web Dashboard\n")
